...
class PriestsTable
{
        ...
        return $table
            ...
            ->columns([
                ImageColumn::make('image_path')
                    ->label('Foto')
                    ->disk('public')
                    ->state(fn (Priest $record): ?string => self::resolveImageState($record->image_path))
                    ->imageSize(40)
                    ->circular(),
    ...
    private static function resolveImageState(?string $imagePath): ?string
    {
        return PublicImagePathResolver::resolveExistingState($imagePath, [
            fn (string $path): string => str_replace('inventario-legacy/sacerdotes/', 'inventory-legacy/priests/', $path),
            fn (string $path): string => str_replace('inventory-legacy/priests/', 'inventario-legacy/sacerdotes/', $path),
        ]);
    }
}
